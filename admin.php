...
<body class="page-admin">
    <div class="container">
        <div class="header">
            ...
            <p class="subtitle">Управление заявками на курсы обучения водителей пассажирского транспорта</p>
        </div>
        ...
        <div class="stats-grid">
            ...
            <a href="<?= admin_url(['status' => 'Идет обучение']) ?>" class="stat-card<?= $f_status === 'Идет обучение' ? ' active' : '' ?>">
                <div class="stat-number" style="color: #0c5460;"><?= $stats['in_progress'] ?></div>
                <div class="stat-label">🎓 Идет обучение</div>
            </a>
            <a href="<?= admin_url(['status' => 'Обучение завершено']) ?>" class="stat-card<?= $f_status === 'Обучение завершено' ? ' active' : '' ?>">
                <div class="stat-number" style="color: #155724;"><?= $stats['completed'] ?></div>
                <div class="stat-label">✅ Обучение завершено</div>
            </a>
        </div>

        <!-- Дополнительные фильтры: вид транспорта, пользователь + сброс -->
        <form method="GET" class="filters-bar">
            <?php if ($f_status !== ''): ?>
                <input type="hidden" name="status" value="<?= htmlspecialchars($f_status) ?>">
            <?php endif; ?>
            <div class="filter-group">
                <label for="f-venue"><i class="fas fa-bus"></i> Вид транспорта</label>
            ...
            <div class="filter-group">
                <label for="f-user"><i class="fas fa-user"></i> Слушатель</label>
                <select id="f-user" name="user_id" onchange="this.form.submit()">
                    <option value="0">Все слушатели</option>
        ...
        <div class="sort-bar">
            ...
            <a href="<?= sort_link('venue')  ?>" class="sort-btn<?= $sort === 'venue'  ? ' active' : '' ?>">Транспорт <?= sort_caret('venue')  ?></a>
            <a href="<?= sort_link('status') ?>" class="sort-btn<?= $sort === 'status' ? ' active' : '' ?>">Статус <?= sort_caret('status') ?></a>
            <a href="<?= sort_link('user')   ?>" class="sort-btn<?= $sort === 'user'   ? ' active' : '' ?>">Слушатель <?= sort_caret('user')   ?></a>
        ...
        <div class="requests-container">
            ...
            ?>
                <div class="empty-state">
                    ...
                    <p>Когда слушатели подадут заявки на курсы обучения, они появятся здесь</p>
                </div>
            <?php } else {
                ...
                <div class="request-item">
                    ...
                    <div class="request-details">
                        <div class="detail-item">
                            <div class="detail-label"><i class="far fa-calendar-alt"></i> Дата и время старта</div>
                            <div class="detail-value"><?= htmlspecialchars($request['date']) ?></div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label"><i class="fas fa-bus"></i> Вид транспорта</div>
                        ...
                        <div class="detail-item">
                            <div class="detail-label"><i class="fas fa-phone"></i> Контакт слушателя</div>
                            <div class="detail-value"><?= htmlspecialchars($request['phone'] ?? '—') ?><?php if (!empty($request['email'])): ?><br><small><?= htmlspecialchars($request['email']) ?></small><?php endif; ?></div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label"><i class="fas fa-cake-candles"></i> Дата рождения</div>
                            <div class="detail-value"><?= !empty($request['birthdate']) ? htmlspecialchars($request['birthdate']) : '—' ?></div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label"><i class="fas fa-comment"></i> Доп. информация от слушателя</div>
                            <div class="detail-value"><?= !empty($request['comment']) ? htmlspecialchars($request['comment']) : '—' ?></div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label"><i class="fas fa-star"></i> Отзыв о курсах</div>
